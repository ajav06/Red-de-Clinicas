...
public class ControladorVtnCitasMedico implements ActionListener{
	...
	private void consultarPaciente() {
		try {
			...
			if (fila == -1) {
				vtnCitas.mostrarMensaje("Seleccione un paciente del listado para Ver su Informaci�n.");
			} else {
	...
	private void buscarCitasMedico() {
		try {
			if("".equals(vtnCitas.getTextField_Cedula())) {
				vtnCitas.mostrarMensaje("Por Favor, Ingrese primero el Nro de Cedula del M�dico, para poder Continuar");
			}
			else {
				MedicoBD medicoDB = new MedicoBD();
				Medico medico = medicoDB.buscarMedico(vtnCitas.getTextField_Cedula());
				if(medico==null) {
					vtnCitas.mostrarMensaje("Por Favor, Ingrese primero un Nro de Cedula del M�dico valido, "
					...
					vtnCitas.mostrarMensaje("Bienvenido, Dr./Dra. "+medico.getNombre()+" "+medico.getApellido()+", "
							+ "\na continuaci�n su agenda de Citas del D�a de Hoy");
